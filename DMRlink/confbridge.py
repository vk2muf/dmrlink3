#!/usr/bin/env python3
#
# Converted to Python 3 by VK2MUF
###############################################################################
...
class confbridgeIPSC(IPSC):
    def __init__(self, _name, _config, _logger, _report):
        ...
        self.STATUS = {
            1: {'RX_TGID':b'\x00', 'TX_TGID':b'\x00', 'RX_TIME':0, 'TX_TIME':0, 'RX_SRC_SUB':b'\x00', 'TX_SRC_SUB':b'\x00'},
            2: {'RX_TGID':b'\x00', 'TX_TGID':b'\x00', 'RX_TIME':0, 'TX_TIME':0, 'RX_SRC_SUB':b'\x00', 'TX_SRC_SUB':b'\x00'}
        }
    ...
    def group_voice(self, _src_sub, _dst_group, _ts, _end, _peerid, _data):
        ...
        _burst_data_type = _data[30:31] # Byte slice
        _seq_id = _data[5:6]
                    ...
                    for _target in BRIDGES[_bridge]:
                        if _target['SYSTEM'] != self._system:
                            if _target['ACTIVE']:                          
                                ...
                                _call_info = _data[17] # indexing bytes returns int
                                if _target['TS'] == 1:
                                    _call_info &= ~(1 << 5)
                                elif _target['TS'] == 2:
                                    _call_info |= 1 << 5
                                ...
                                if _burst_data_type == BURST_DATA_TYPE['SLOT1_VOICE'] or _burst_data_type == BURST_DATA_TYPE['SLOT2_VOICE']:
                                    if _target['TS'] == 1:
                                        _target_burst_type = BURST_DATA_TYPE['SLOT1_VOICE']
                                    else:
                                        _target_burst_type = BURST_DATA_TYPE['SLOT2_VOICE']
                                    _tmp_data = _tmp_data[:30] + _target_burst_type + _tmp_data[31:]
        ...
        if _burst_data_type == BURST_DATA_TYPE['VOICE_HEAD']:
            if self.last_seq_id != _seq_id or (self.call_start + TS_CLEAR_TIME) < now:
                ...
                if self._CONFIG['REPORTS']['REPORT_NETWORKS'] == 'NETWORK':
                    self._report.send_bridgeEvent('({}) GROUP VOICE START: CallID: {} PEER: {}, SUB: {}, TS: {}, TGID: {}'.format(self._system, int_id(_seq_id), int_id(_peerid), int_id(_src_sub), _ts, int_id(_dst_group)).encode('utf-8'))
        ...
        if _burst_data_type == BURST_DATA_TYPE['VOICE_TERM']:
            if self.last_seq_id == _seq_id:
                ...
                if self._CONFIG['REPORTS']['REPORT_NETWORKS'] == 'NETWORK':
                    self._report.send_bridgeEvent('({}) GROUP VOICE END:   CallID: {} PEER: {}, SUB: {}, TS: {}, TGID: {} Duration: {:.2f}s'.format(self._system, int_id(_seq_id), int_id(_peerid), int_id(_src_sub), _ts, int_id(_dst_group), self.call_duration).encode('utf-8'))
            else:
                ...
                if self._CONFIG['REPORTS']['REPORT_NETWORKS'] == 'NETWORK':
                    self._report.send_bridgeEvent('(%s) GROUP VOICE END WITHOUT MATCHING START:   CallID: %s PEER: %s, SUB: %s, TS: %s, TGID: %s'.format(self._system, int_id(_seq_id), int_id(_peerid), int_id(_src_sub), _ts, int_id(_dst_group)).encode('utf-8'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config', action='store', dest='CFG_FILE', help='/full/path/to/config.file')
    ...
    logger.info('DMRlink Python 3 Start - SYSTEM STARTING...')
