...
import configparser  # Renamed from ConfigParser
import sys
import binascii     # Used for the new way to handle hex decoding
...
def build_config(_config_file):
    ...
    if not config.read(_config_file):
            sys.exit(f"Configuration file '{_config_file}' is not a valid configuration file! Exiting...")        
        ...
        for section in config.sections():
            if section == 'GLOBAL':
                ...
                if CONFIG['REPORTS']['REPORT_RCM']:
                    # In Py3, ensure this is a proper boolean
                    CONFIG['REPORTS']['REPORT_RCM'] = config.getboolean(section, 'REPORT_RCM')
                ...
                # Radio ID and Auth Key conversion using binascii
                radio_id_hex = hex(int(config.get(section, 'RADIO_ID')))[2:].rjust(8,'0')
                auth_key_hex = config.get(section, 'AUTH_KEY').rjust(40,'0')
                ...
                CONFIG['SYSTEMS'][section]['LOCAL'].update({
                    ...
                    'RADIO_ID':     binascii.unhexlify(radio_id_hex),
                ...
                # Bitwise logic for MODE and FLAGS
                MODE_BYTE = 0
                FLAG_1 = 0
                FLAG_2 = 0
                ...
                # Use bytes([int]) to create a byte object in Python 3
