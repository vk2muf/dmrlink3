...
def process_flags_bytes(_hex_flags):
    _byte3 = int(ahex(_hex_flags[2:3]), 16)
    _byte4 = int(ahex(_hex_flags[3:4]), 16)
...
def build_peer_list(_peers):
    ...
    peer_list = bytes_2(len(concatenated_peers)) + concatenated_peers
...
class IPSC(DatagramProtocol):
    ...
    def process_peer_list(self, _data):
        ...
        for i in range(7, _peer_list_length +7, 11):
            ...
            if _hex_radio_id in self._peers.keys():
                ...
                self._peers[_hex_radio_id]['FLAGS_DECODE'] = b''
    ...
    def call_mon_status(self, _data):
        ...
        if self._rcm:
            self._report.send_rcm(self._system.encode('ascii') + b',' + _data)
    ...
    #
    def send_packet(self, _packet, host_port):
        _host, _port = host_port
        if self._local['AUTH_ENABLED']:
            _hash = bhex((hmac_new(self._local['AUTH_KEY'],_packet,sha1)).hexdigest()[:20].encode('ascii'))
    ...
    def master_reg_req(self, _data, _peerid, _host, _port):
        ...
        self.MASTER_REG_REPLY_PKT = (MASTER_REG_REPLY + self._local_id + self.TS_FLAGS + bytes_2(self._local['NUM_PEERS']) + IPSC_VER)
    ...
    def hashed_packet(self, _key, _data):
        _hash = bhex((hmac_new(_key,_data,sha1)).hexdigest()[:20].encode('ascii'))
        return _data + _hash
    ...
    def validate_auth(self, _key, _data):
        ...
        _hash = _data[-10:]
        _chk_hash = bhex((hmac_new(_key,_payload,sha1)).hexdigest()[:20].encode('ascii'))
