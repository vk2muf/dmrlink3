...
def encode(_msg):
    # _msg is expected to be a bytes-like object (e.g., bytearray)
    assert len(_msg) == 9, 'RS129_encode error: Message not 9 bytes'
...
def lc_header_encode(_message):
    # Ensure _message is bytes if passed as a string
    if isinstance(_message, str):
        _message = _message.encode('latin-1')
    bin_message = bytearray(_message)
    parity = encode(bin_message)
    masked_parity = lc_header_mask(parity)
    return bytes(masked_parity)
...
if __name__ == '__main__':    
    ...
    print('Original Message:            {}'.format(ahex(message).decode()))
    print('Masked Parity Should be:     {}'.format(ahex(parity_should_be).decode()))
    
    parity = lc_header_encode(message)
    print('Calculated Masked Parity is: {}'.format(ahex(parity).decode()))