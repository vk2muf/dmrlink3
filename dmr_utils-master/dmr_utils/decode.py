...
if __name__ == '__main__':
    ...
    # SAMPLE, KNOWN GOOD DMR BURSTS - Converted to bytes literals for Py3
    ...
    print('LC: OPT-{} SRC-{} DST-{}, SLOT TYPE: CC-{} DTYPE-{}'.format(
        ahex(lc['LC'][0:3]).decode('ascii'),
        ahex(lc['LC'][3:6]).decode('ascii'),
        ahex(lc['LC'][6:9]).decode('ascii'),
        ahex(lc['CC']).decode('ascii'),
        ahex(lc['DTYPE']).decode('ascii')
    ))
    print('Decode Time: {}\n'.format(t1-t0))
    ...
    t1 = time()
    print('VOICE SYNC: {}'.format(ahex(lc['SYNC'].tobytes()).decode('ascii')))
    ...
    embed_lc += pkt['EMBED']
    t1 = time()
    print('EMB: CC-{} LCSS-{}, EMBEDDED LC: {}'.format(
        ahex(pkt['CC']).decode('ascii'), 
        ahex(pkt['LCSS']).decode('ascii'), 
        ahex(pkt['EMBED'].tobytes()).decode('ascii')
    ))
    print('AMBE 0: {}, {}'.format(pkt['AMBE'][0], len(pkt['AMBE'][0])))
    print('AMBE 1: {}, {}'.format(pkt['AMBE'][1], len(pkt['AMBE'][1])))
    print('AMBE 2: {}, {}'.format(pkt['AMBE'][1], len(pkt['AMBE'][2])))
    ...
    print('COMPLETE EMBEDDED LC: {}'.format(ahex(embed_lc).decode('ascii')))
    print('AMBE 0: {}, {}'.format(pkt['AMBE'][0], len(pkt['AMBE'][0])))
    print('AMBE 1: {}, {}'.format(pkt['AMBE'][1], len(pkt['AMBE'][1])))
    print('AMBE 2: {}, {}'.format(pkt['AMBE'][1], len(pkt['AMBE'][2])))
    print(t1-t0, '\n')
