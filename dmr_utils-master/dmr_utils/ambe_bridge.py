...
BitString = BitStream # Fallback alias for legacy compatibility
import struct
from time import time, sleep, localtime, strftime
# clock() was removed in Python 3.8, time() or perf_counter() are used now.
from time import perf_counter as clock 
from importlib import import_module
from binascii import b2a_hex as ahex
from random import randint
import socket, configparser, traceback
...
from . import ambe_utils

# Does anybody read this stuff? There's a PEP somewhere that says I should do this.
__author__     = 'Mike Zingman, N4IRR and Cortney T. Buffington, N0MJS, VK2MUF'
...
# Constants
############################################################################################################
DMR_DATA_SYNC_MS    =   b'\xD5\xD7\xF7\x7F\xD7\x57'
...
'''
# Extract int from const string if necessary
header_flag     = lambda _slot: (0xA0 if (_slot == 2) else 0x20) | (const.DMR_SLT_VHEAD[0] if isinstance(const.DMR_SLT_VHEAD, bytes) else ord(const.DMR_SLT_VHEAD))
terminator_flag = lambda _slot: (0xA0 if (_slot == 2) else 0x20) | (const.DMR_SLT_VTERM[0] if isinstance(const.DMR_SLT_VTERM, bytes) else ord(const.DMR_SLT_VTERM))
...
class AMBE_BASE:
    ...
    def play_ambe_file(self, _fileName, _rx_slot):
        try:
            ...
            _strSlot = struct.pack("I",_rx_slot.slot)[:1]
            metadata = ahex(_rx_slot.rf_src[0:3]).decode('ascii') + ahex(_rx_slot.repeater_id[0:4]).decode('ascii') + ahex(_rx_slot.dst_id[0:3]).decode('ascii') + ('%02x' % _rx_slot.slot) + ('%02x' % _rx_slot.cc)
    ...
    def import_datagramReceived(self, _data, addr):
        _host, _port = addr
        ...
        if len(_data) >= 2:
            ...
            if (t == TAG_BEGIN_TX) or (t == TAG_SET_INFO):
                ...
                _rx_slot.dst_id = hex_str_3(int(v.decode('ascii').split('=')[1]))
                ...
                thread.start_new_thread( self.play_ambe_file, (v.decode('ascii').split('=')[1], _rx_slot) )
            elif (t == TAG_REMOTE_CMD):
                _tmp = v.decode('ascii').split(None)[0] #first get rid of whitespace
                _cmd = _tmp.split('=')[0]
                if _cmd == "foobar":
                    pass
                elif _cmd == 'get_info':    # get section name, repeater ID, subscriber ID, subscriber callsign
                    reply_msg = 'reply dmr_info {} {} {} {}'.format(self._system,
                                                                          int_id(_rx_slot.repeater_id),
                                                                          int_id(_rx_slot.rf_src),
                                                                          get_alias(_rx_slot.rf_src, subscriber_ids))
                    self._sock.sendto(reply_msg.encode('ascii'), (self._dmrgui, 34003))
                    ...
                    self._slot = int(v.decode('ascii').split('=')[1])
                elif _cmd == 'gateway_dmr_id':
                    id = int(v.decode('ascii').split('=')[1])
                    ...
                    self._logger.info('(%s) unknown remote command: %s', self._system, v.decode('ascii', errors='ignore'))
                    
            else:
                self._logger.info('(%s) unknown TLV t=%d, l=%d, v=%s', self._system, t, l, ahex(v).decode('ascii'))
        else:
            self._logger.info('(%s) EOF or malformed UDP stream', self._system)
    ...
    def begin_call(self, _slot, _src_id, _dst_id, _repeater_id, _cc, _seq, _stream_id):
        ...
        reply_msg = 'reply log2 {} {}'.format(_src_alias, int_id(_dst_id))
        self._sock.sendto(reply_msg.encode('ascii'), (self._dmrgui, 34003))
    ...
    def end_call(self, _tx_slot):
        ...
        log_msg = "reply log" + strftime(" %m/%d/%y %H:%M:%S", localtime(_tx_slot.start_time)) + \
                  ...
                  ' {:.2f}s'.format(call_duration)
        self._sock.sendto(log_msg.encode('ascii'), (self._dmrgui, 34003))
...
class AMBE_HB(AMBE_BASE):
    ...
    def send_voice72(self, _rx_slot, _ambe):
        ...
        _new_frame = self.encode_voice( BitArray('0x'+ahex(_ambe).decode('ascii')), _rx_slot )     # Construct the dmr frame from AMBE(108 bits) + sync/CACH (48 bits) + AMBE(108 bits)
    ...
    def send_voice49(self, _rx_slot, _ambe):
        ambe49_1 = BitArray('0x' + ahex(_ambe[0:7]).decode('ascii'))[0:49]
        ambe49_2 = BitArray('0x' + ahex(_ambe[7:14]).decode('ascii'))[0:49]
        ambe49_3 = BitArray('0x' + ahex(_ambe[14:21]).decode('ascii'))[0:49]
        ...
        self.send_voice72(_rx_slot, v.tobytes())
    ...
    def make_dmrd( self, _seq, _rf_src, _dst_id, _repeater_id, _flag, _stream_id, _dmr_frame):
        frame = bytearray(b'DMRD')                  # HB header type DMRD
        frame += struct.pack("i", _seq)[0:1]        # add sequence number (Python 3 slicing ensures bytes are returned)
    ...
    def __encode_voice_header( self, _rx_slot, _sync, _dtype ):
        ...
        _cc = _rx_slot.cc
        # create lc
        lc = b'\x00\x00\x00' + _dst_id + _src_id        # PF + Reserved + FLCO + FID + Service Options + Group Address + Source Address
        # encode lc into info
        ...
        # create slot_type
        slot_type = bytes([(_cc << 4) | (_dtype & 0x0f)]) # data type is Header or Term
...
class AMBE_IPSC(AMBE_BASE):
    ...
    def send_voice72(self, _rx_slot, _ambe):
        ambe72_1 = BitArray('0x' + ahex(_ambe[0:9]).decode('ascii'))[0:72]
        ambe72_2 = BitArray('0x' + ahex(_ambe[9:18]).decode('ascii'))[0:72]
        ambe72_3 = BitArray('0x' + ahex(_ambe[18:27]).decode('ascii'))[0:72]
    ...
    def send_voice49(self, _rx_slot, _ambe):
        ambe49_1 = BitArray('0x' + ahex(_ambe[0:7]).decode('ascii'))[0:50]
        ambe49_2 = BitArray('0x' + ahex(_ambe[7:14]).decode('ascii'))[0:50]
        ambe49_3 = BitArray('0x' + ahex(_ambe[14:21]).decode('ascii'))[0:50]
        ambe = ambe49_1 + ambe49_2 + ambe49_3
    ...
    def rewriteFrame( self, _frame, _newSlot, _newGroup, _newSourceID, _newPeerID ):
        ...
        _frame = _frame[:5] + struct.pack("i", self.ipsc_seq[_newSlot])[0:1] + _frame[6:]   # ipsc sequence number increments on each transmission (stream id)
        ...
        if _newSlot == 1:
            ...
            _call_info |= 1 << 5
        _call_info = bytes([_call_info])
        _frame = _frame[:17] + _call_info + _frame[18:]
    
        _x = struct.pack("i", self._seq[_newSlot])
        _frame = _frame[:20] + _x[1:2] + _x[0:1] + _frame[22:]          # rtp sequence number increments for EACH frame sent out
...
class TEST_HARNESS:
    ...
    def send_system(self, _frame):
        print('send system', ahex(_frame).decode('ascii'),'\n')
    def send_to_ipsc(self, _frame):
        print('send_to_ipsc', ahex(_frame).decode('ascii'),'\n')
