...
def encode_19696(_data):
    # Create a bitarray from the LC data (includes RS1293 ECC)
...
# Accepts 12 byte LC header + 5-bit checksum, converts to binary and builds out the BPTC
...
def encode_emblc(_lc):
    ...
    # Create a bitarray from the LC data (includes 5-bit checksum).
    _binlc = bitarray(endian='big')
    _binlc.frombytes(_lc)
    
    # Insert the checksum bits at the right location in the matrix
    ...
    emblc_d.extend([_binlc[9], _binlc[25],_binlc[41],_binlc[57],_binlc[73],_binlc[89],_binlc[105],_binlc[121]]) # Corrected index 25
...
if __name__ == '__main__':
    ...
    # Header LC -- Terminator similar. Note: Must be bytes in Py3
    ...
    print('Orig Data:     {}, {} bytes'.format(ahex(lc).decode('ascii'), len(lc)))
    print('Orig Encoded:  {}, {} bytes'.format(ahex(voice_hb.tobytes()).decode('ascii'), len(voice_hb.tobytes())))
    print()
    print('BPTC(196,96):')
    print('Encoded data:  {}, {} bytes'.format(ahex(full_lc_encode.tobytes()).decode('ascii'), len(full_lc_encode.tobytes())))
    print('Encoding time: {} seconds'.format(encode_time))
    print('Decoded data:  {}'.format(ahex(full_lc_dec.tobytes()).decode('ascii')))
    ...
    print('\nEMBEDDED LC:')
    print('Encoded Data:  Burst B:{} Burst C:{} Burst D:{} Burst E:{}'.format(
        ahex(emblc[1].tobytes()).decode('ascii'), 
        ahex(emblc[2].tobytes()).decode('ascii'), 
        ahex(emblc[3].tobytes()).decode('ascii'), 
        ahex(emblc[4].tobytes()).decode('ascii')
    ))
    print('Encoding Time: {}'.format(encode_time))
    print('Decoded data:  {}'.format(ahex(decemblc).decode('ascii')))
