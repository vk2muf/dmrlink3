    ...
    def dmrd_received(self, _radio_id, _rf_src, _dst_id, _seq, _slot, _call_type, _frame_type, _dtype_vseq, _stream_id, _data):
        ...
        if _call_type == 'group':
            ...
            if (_stream_id != self.STATUS[_slot]['RX_STREAM_ID']):
                ...
                self.STATUS[_slot]['RX_START'] = pkt_time # Added missing slot index
                            ...
                            if _target['SYSTEM'] != self._system:
                                if _target['ACTIVE']:                             
                                    ...
                                    if ((_target['TGID'] != _target_status[_target['TS']]['RX_TGID']) and ((pkt_time - _target_status[_target['TS']]['RX_TIME']) < _target_system['GROUP_HANGTIME'])):
                                        if _frame_type == hb_const.HBPF_DATA_SYNC and _dtype_vseq == hb_const.HBPF_SLT_VHEAD and self.STATUS[_slot]['RX_STREAM_ID'] != _seq: # Note: _seq integer check
                                    ...
                                    # Assemble transmit HBP packet header - fixed chr() to bytes() for Python 3
                                    _tmp_data = _data[:8] + _target['TGID'] + _data[11:15] + bytes([_tmp_bits]) + _data[16:20]
