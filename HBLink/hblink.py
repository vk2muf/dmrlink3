...
try:
    from dmr_utils.utils import bytes_4
except ImportError:
...
#Temp fix
# --- PYTHON 3 LEGACY COMPATIBILITY SHIM START ---
import binascii

def bytes_2(_int_id):
    return binascii.unhexlify(format(_int_id, 'x').zfill(4))

def bytes_3(_int_id):
    return binascii.unhexlify(format(_int_id, 'x').zfill(6))

def bytes_4(_int_id):
    return binascii.unhexlify(format(_int_id, 'x').zfill(8))

def int_id(_hex_string):
    # If it's already an int (Python 3 indexing behavior), just return it
    if isinstance(_hex_string, int):
        return _hex_string
...
# --- PYTHON 3 LEGACY COMPATIBILITY SHIM END ---



# Does anybody read this stuff? There's a PEP somewhere that says I should do this.
__author__     = 'Cortney T. Buffington, N0MJS, VK2MUF'
...
class HBSYSTEM(DatagramProtocol):
    def __init__(self, _name, _config, _logger):
        ...
        if self._config.get('EXPORT_AMBE'):
    ...
    def master_maintenance_loop(self):
        ...
        # We use list() to create a copy of keys because we may delete items during iteration
    ...
    def client_maintenance_loop(self):
        ...
        if self._stats['CONNECTION'] == 'NO' or self._stats['CONNECTION'] == 'RPTL_SENT' or self._stats['NUM_OUTSTANDING'] >= self._CONFIG['GLOBAL']['MAX_MISSED']:
            ...
            _packet = b'RPTL' + self._config['RADIO_ID']
            self.send_master(_packet)
        ...
        if self._stats['CONNECTION'] == 'YES':
            _packet = b'RPTPING' + self._config['RADIO_ID']
            self.send_master(_packet)
    ...
    def send_client(self, _client, _packet):
        ...
        if isinstance(_packet, str):
            _packet = _packet.encode('latin-1')
    ...
    def client_dereg(self):
        _packet = b'RPTCL' + self._config['RADIO_ID']
        self.send_master(_packet)
    ...
    def master_datagramReceived(self, _data, addr):
        _host, _port = addr
        ...
        if _command == b'DMRD':    # DMRData -- encapsulated DMR data frame
                ...
                if self._config.get('REPEAT') == True:
                    for _client in self._clients:
                        if _client != _radio_id:
                            _out_data = _data[0:11] + _client + _data[15:]
                            self.send_client(_client, _out_data)
        ...
        elif _command == b'RPTL':    # RPTLogin -- a repeater wants to login
            _radio_id = _data[4:8]
            if _radio_id:           # Future check here for valid Radio ID
                ...
                _salt_str = bytes_4(self._clients[_radio_id]['SALT'])
                _packet = b'RPTACK' + _salt_str
                self.send_client(_radio_id, _packet)
        ...
        elif _command == b'RPTK':    # Repeater has answered our login challenge
            _radio_id = _data[4:8]
            if _radio_id in self._clients \
                ...
                _sent_hash = _data[8:]
                _salt_str = bytes_4(_this_client['SALT'])
                # Passphrase must be bytes for hashing
                _passphrase = self._config['PASSPHRASE']
                if isinstance(_passphrase, str):
                    _passphrase = _passphrase.encode('ascii')
                    _calc_hash = bhex(sha256(_salt_str + (_passphrase if isinstance(_passphrase, bytes) else _passphrase.encode('ascii'))).hexdigest().encode('ascii'))
                if _sent_hash == _calc_hash:
                    _this_client['CONNECTION'] = 'WAITING_CONFIG'
                    _packet = b'RPTACK' + _radio_id
                    self.send_client(_radio_id, _packet)
                    self._logger.info('(%s) Client %s has completed the login exchange successfully', self._system, _this_client['RADIO_ID'])
                else:
                    self._logger.info('(%s) Client %s has FAILED the login exchange', self._system, _this_client['RADIO_ID'])
                    _packet = b'MSTNAK' + _radio_id
                    self.transport.write(_packet, (_host, _port))
        ...
        elif _command == b'RPTC':    # Repeater is sending it's configuraiton OR disconnecting
            if _data[:5] == b'RPTCL':    # Disconnect command
                _radio_id = _data[5:9]
                if _radio_id in self._clients \
                            and self._clients[_radio_id]['CONNECTION'] == 'YES' \
                            and self._clients[_radio_id]['IP'] == _host \
        ...
        elif _command == b'RPTP':    # RPTPing -- client is pinging us
            ...
            self._logger.error('(%s) Unrecognized command. Raw HBP PDU: %s', self._system, ahex(_data).decode('ascii'))
        
    # Aliased in __init__ to datagramReceived if system is a client
    def client_datagramReceived(self, _data, addr):
        _host, _port = addr
        # Validate that we received this packet from the master - security check!
        if self._config['MASTER_IP'] == _host and self._config['MASTER_PORT'] == _port:
            ...
            if   _command == b'DMRD':    # DMRData -- encapsulated DMR data frame
                _radio_id = _data[11:15]
                if self._config.get('LOOSE') or _radio_id == self._config['RADIO_ID']: # Validate the Radio_ID unless using loose validation
                    _seq = _data[4:5]
                    _rf_src = _data[5:8]
                    _dst_id = _data[8:11]
                    _bits = int_id(_data[15])
            ...
            elif _command == b'MSTN':    # Actually MSTNAK -- a NACK from the master
            ...
            elif _command == b'RPTA':    # Actually RPTACK -- an ACK from the master
                ...
                if self._stats['CONNECTION'] == 'RPTL_SENT': # If we've sent a login request...
                    ...
                    if isinstance(_passphrase, str):
                        _passphrase = _passphrase.encode('ascii')
                    _pass_hash = sha256(_login_int32 + _passphrase).hexdigest()
                    _pass_hash = bhex(_pass_hash)
                    _packet = b'RPTK' + self._config['RADIO_ID'] + _pass_hash
                    self.send_master(_packet)
                    ...
                    if self._config.get('LOOSE') or _radio_id == self._config['RADIO_ID']: # Validate the Radio_ID unless using loose validation
                        ...
                        self.send_master(b'RPTC' + _config_packet)
                    ...
                    if self._config.get('LOOSE') or _radio_id == self._config['RADIO_ID']: # Validate the Radio_ID unless using loose validation
                        if self._config.get('OPTIONS'):
                            self.send_master(b'RPTO' + self._config['RADIO_ID'] + self._config['OPTIONS'])
            ...
            elif _command == b'MSTP':    # Actually MSTPONG -- a reply to RPTPING (send by client)
                _radio_id = _data[7:11]
                if self._config.get('LOOSE') or _radio_id == self._config['RADIO_ID']: # Validate the Radio_ID unless using loose validation
                    self._stats['PING_OUTSTANDING'] = False
                    self._stats['NUM_OUTSTANDING'] = 0
            ...
            elif _command == b'MSTC':    # Actually MSTCL -- notify us the master is closing down
                _radio_id = _data[5:9]
                if self._config.get('LOOSE') or _radio_id == self._config['RADIO_ID']: # Validate the Radio_ID unless using loose validation
                    self._stats['CONNECTION'] = 'NO'
                ...
                self._logger.error('(%s) Received an invalid command in packet: %s', self._system, ahex(_data).decode('ascii'))
